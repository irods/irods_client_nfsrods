...
public class IRODSVirtualFileSystemTest {
	...
	/*
	 * @Test public void testForceRemoveEmptyDirectory() throws Exception{ //get
	 * irods acct stuff ready IRODSAccount irodsAccount =
	 ...
	 * irodsFileSystem.getIRODSAccessObjectFactory(); String homeDir =
	 * MiscIRODSUtils.buildIRODSUserHomeForAccountUsingDefaultScheme(irodsAccount);
	 * IRODSFile rootFile =
	 * accessObjectFactory.getIRODSFileFactory(irodsAccount).instanceIRODSFile(
	 * homeDir);
	 * 
	 * //create VFS IrodsVirtualFileSystem vfs = new
	 * IrodsVirtualFileSystem(accessObjectFactory, irodsAccount, rootFile);
	 * 
	 * //create Test Dir string String path = "testRemoveEmptyDirectory";
	 * 
	 * //get subject for mkdir() Subject currentUser = UnixUtils.getCurrentUser();
	 * 
	 * //get root inode Inode root = vfs.getRootInode();
	 * 
	 * //call mkdir() vfs.mkdir(root, path, currentUser, 0);
	 * 
	 * //remove vfs.remove(root, path);
	 * 
	 * 
	 * //confirm its in trash
	 * 
	 * //check if file is in trash directory TrashOperationsAO trashOperationsAO =
	 * irodsFileSystem.getIRODSAccessObjectFactory()
	 * .getTrashOperationsAO(irodsAccount); IRODSFile file =
	 * trashOperationsAO.getTrashHomeForLoggedInUser();
	 * 
	 * //get list of trash files TODO: Add filte rfor list String[] files =
	 * file.list(); if(Arrays.asList(files).contains(path)){
